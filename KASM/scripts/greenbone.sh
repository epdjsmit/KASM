#!/bin/bash
export PATH=$PATH:/usr/local/sbin && export INSTALL_PREFIX=/usr/local && export SOURCE_DIR=$HOME/source && mkdir -p $SOURCE_DIR && export BUILD_DIR=$HOME/build && mkdir -p $BUILD_DIR && export INSTALL_DIR=$HOME/install && mkdir -p $INSTALL_DIR
sudo apt update
...
sudo apt install -y libglib2.0-dev libgpgme-dev libgnutls28-dev uuid-dev libssh-gcrypt-dev libhiredis-dev libxml2-dev libpcap-dev libnet1-dev libpaho-mqtt-dev libldap2-dev libradcli-dev
